				...
				<form class="login100-form validate-form" method="POST" action="">
					...
					<div class="container-login100-form-btn">
						<div class="wrap-login100-form-btn">
							...
							<button onclick='login.php' class="login100-form-btn">
								Sign Up
							</button>
						</div>
					</div>
...
<?php
    ...
    if (isset($_REQUEST['email'])){
		...
		//$PolaEmail = "/([a-zA-Z0-9\-\.\_])(@[gmail|ymail|yahoo|outlook])(\.[com|net|id|co.id])/";
		...
		if(filter_var($email, FILTER_VALIDATE_EMAIL)){
			...
			if(mysqli_num_rows($row)==0 && mysqli_num_rows($row_nohp)==0){
					$query = "INSERT into `user` (Nama_user, Email_user, NoHp_user, Password) VALUES ('$nama', '$email','$nohp','$password')";
					$result = mysqli_query($con,$query);	
					echo "<script>alert('Registrasi berhasil')</script>";
					echo "<script>window.open('/ebuss/login.php','_self')</script>";
				}
			
			if(mysqli_num_rows($row)>0){
				echo "<script>alert('Registrasi Gagal. Email Telah Terpakai!')</script>";
				echo "<script>window.history.back()</script>";
			}	
			if(mysqli_num_rows($row_nohp)>0){
				echo "<script>alert('Registrasi Gagal. No HP Telah Terpakai!')</script>";
				echo "<script>window.history.back()</script>";
			}			
		}		
    }
